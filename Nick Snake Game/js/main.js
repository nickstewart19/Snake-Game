...
var pixel = 20;
var corDoJogo = [
  { cobra: "black", fundo: "#848583", fruta: "red" },
...
function seComeuAComida() {
  ...
  if (cobra.posX == comida.posX && cobra.posY == comida.posY) {
    ...
    for (var i = 0; i < cobra.cauda.length; i++) {
      if (
        (cobra.cauda[i].x == comida.posX && cobra.cauda[i].y == comida.posY) ||
        (cobra.posX == comida.posX && cobra.posY == comida.posY)
      ) {
        gerarComida();
        i = 0;
...
function seBateuNaCauda() {
  cobra.cauda.map(function (posicaoCauda) {
    if (posicaoCauda.x == cobra.posX && posicaoCauda.y == cobra.posY)
      gameOver();
  });
}
...
var graficos = {
  ...
  desenharCenario: function () {
    this.desenharRetangulo(
      corDoJogo[numeroDoCenario - 1].fundo,
      0,
      0,
      cenario.width,
      cenario.height
  ...
  desenharCobra: function () {
    var that = this;
    cobra.cauda.map(function (posicaoCauda) {
      that.desenharRetangulo(
        corDoJogo[numeroDoCenario - 1].cobra,
        posicaoCauda.x,
        posicaoCauda.y,
        pixel - 1,
        pixel - 1
  ...
  desenharComida: function () {
    this.desenharArco(
      corDoJogo[numeroDoCenario - 1].fruta,
      comida.posX + 10,
      comida.posY + 10,
      10,
      0,
